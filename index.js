...
const app = express()
    ...
    const neck_traits = ["No Neck Accessory","Gold","Silver","Diamond","Gold MP","Gold Link","Dollar Sign","Silver Link","Bitcoin",
                            "Red Sling Bag","Black Sling Bag","Green Sling Bag"]
...
const rarity = async (cid,count)=>{
    ...
    let NECK = address[0].NECK.split(',')
    let score = [];
    let raritydata = await RaritySchema.deleteMany()
    for(i=1;i<=count;i++){
        let rawdata = fs.readFileSync(`./json/${i}.json`);
        let json = JSON.parse(rawdata);
                ...
                1 / (SHIRTS[shirt_traits.indexOf(json.attributes[5].value)]/8888) +
                1 / (HEADWEAR[headwear_traits.indexOf(json.attributes[6].value)]/8888) +
                1 / (FACIAL[facialexpression_traits.indexOf(json.attributes[7].value)]/8888) +
                1 / (EYES[eye_traits.indexOf(json.attributes[8].value)]/8888) +
                1 / (EYEWEAR[eyewear_traits.indexOf(json.attributes[9].value)]/8888)+
                1 / (NECK[neck_traits.indexOf(json.attributes[4].value)]/8888)
                score.push(parseInt(rarityscore* 1000) )
                // score.push(rarityscore)
        if(i == 4554) console.log(rarityscore)
                
    }
    var sorted = score.slice().sort(function(a,b){return b-a})
    var ranks = score.map(function(v){ return sorted.indexOf(v)+1 });
    raritydata = new RaritySchema({score:score.join(',')})
    await raritydata.save()
    rankdata = new RaritySchema({score:ranks.join(',')})
    await rankdata.save()
    console.log(ranks)
    return score;
}
...
const read = async (cid,count)=>{
    ...
    for(i=1;i<=count;i++){
        ...
        let json = JSON.parse(rawdata);
        // console.log(json)
        // await fetch(`./json/${i}.json`,settings)
        //     .then(res => res.json())
        //     .then((json) => {
                    console.log(json.attributes[4].value)
                    ...
                    shirts[shirt_traits.indexOf(json.attributes[5].value)]++
                    head[headwear_traits.indexOf(json.attributes[6].value)]++
                    facial[facialexpression_traits.indexOf(json.attributes[7].value)]++
                    eyes[eye_traits.indexOf(json.attributes[8].value)]++
                    eyewear[eyewear_traits.indexOf(json.attributes[9].value)]++
                    neck[neck_traits.indexOf(json.attributes[4].value)]++
            // });
...
app.get('/rank',async (req,res)=>{
    let raritydata = await RaritySchema.find()
    res.send(raritydata[1].score.split(','))
})
